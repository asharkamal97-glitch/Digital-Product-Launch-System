    ...
    <AdminLayout>
      <div className="space-y-6 max-w-7xl mx-auto animate-fade-in">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white rounded-3xl p-6 border border-slate-200 shadow-sm">
          <div>
            <h1 className="text-2xl font-extrabold text-brand-950">Admissions & Registration Desk</h1>
            <p className="text-xs text-slate-500">
              Process student applications, review verification documents, and update applicant progression.
            </p>
          </div>
                  ...
                  applications.map((app) => (
                    <tr key={app.id} className="hover:bg-slate-50/80 transition-colors">
                      ...
                      <td className="py-3.5 px-4">
                        ...
                        <p className="text-[10px] text-slate-400">{app.gender} • DOB: {formatDate(app.dob)}</p>
                      </td>
          ...
          <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-950/60 backdrop-blur-sm flex items-center justify-center p-4">
            <div className="bg-white rounded-3xl p-6 sm:p-8 max-w-lg w-full border border-slate-200 shadow-2xl space-y-6 animate-fade-in my-8">
              <div className="flex items-center justify-between border-b border-slate-100 pb-3">
                <div>
                  <h3 className="text-lg font-bold text-brand-950">Application Details</h3>
              ...
              <div className="p-4 rounded-2xl bg-slate-50 border border-slate-200 text-xs space-y-2">
                ...
                <p><span className="text-slate-500">Class Applying:</span> <strong className="text-brand-900">{selectedApp.applyingForClass}</strong></p>
                <p><span className="text-slate-500">Parents:</span> {selectedApp.fatherName} & {selectedApp.motherName}</p>
                <p><span className="text-slate-500">Phone:</span> {selectedApp.parentPhone} | Email: {selectedApp.parentEmail}</p>
                <p><span className="text-slate-500">Residential Mode:</span> <strong className="text-emerald-700">{selectedApp.isResidential ? 'Residential Hostel' : 'Day Scholar'}</strong></p>
                <p><span className="text-slate-500">Address:</span> {selectedApp.address}</p>
              </div>

              <div className="space-y-1.5 text-xs">
                <label className="font-bold text-slate-700">Official Administrative Remarks</label>
                <textarea
                  rows={3}
                  value={remarks}
                  onChange={(e) => setRemarks(e.target.value)}
                  placeholder="e.g. Documents verified. Entrance test cleared with 85%."
              ...
              <div className="pt-2 flex flex-wrap gap-2 justify-end border-t border-slate-100 text-xs font-bold">
                <button
                  type="button"
                  disabled={updating}
                  onClick={() => handleStatusUpdate(selectedApp.id, 'DOCUMENTS_REQUIRED')}
                  className="px-3 py-2 rounded-xl bg-purple-100 hover:bg-purple-200 text-purple-900"
                >
                  Need Documents
                </button>
                <button
                  ...
                  className="px-3 py-2 rounded-xl bg-red-100 hover:bg-red-200 text-red-800"
                >
                  Reject
                </button>
                <button
