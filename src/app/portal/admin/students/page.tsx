...
export default function AdminStudentsPage() {
  ...
  const exportCSV = () => {
    const headers = ['Admission No', 'Roll No', 'Name', 'Class', 'Section', 'Gender', 'Father Name', 'Phone', 'Hostel'];
    ...
    <PortalLayout role="ADMIN">
      <div className="space-y-6 max-w-7xl mx-auto animate-fade-in">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white rounded-3xl p-6 border border-slate-200 shadow-sm">
          <div>
            <h1 className="text-2xl font-extrabold text-brand-950">Student Management Directory</h1>
            <p className="text-xs text-slate-500">
              Manage student enrollment profiles, assign roll numbers, view class rosters, and generate ID cards.
            </p>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              onClick={() => {
                setFormData((prev) => ({
                  ...prev,
                  admissionNo: `SARS-2026-${String(students.length + 100).padStart(3, '0')}`,
            ...
            <table className="w-full text-left text-xs">
              <thead>
                <tr className="bg-slate-50 border-b border-slate-200 text-slate-600 font-bold uppercase tracking-wider">
                  ...
                  <th className="py-3 px-4">Hostel Status</th>
                    ...
                    <tr key={st.id} className="hover:bg-slate-50/80 transition-colors">
                      <td className="py-3.5 px-4">
                        <div className="flex items-center gap-3">
                          <div className="w-8 h-8 rounded-full bg-brand-900 text-gold-400 font-bold flex items-center justify-center text-xs">
                            {st.firstName[0]}
                          </div>
                          <div>
                      ...
                      <td className="py-3.5 px-4">
                        ...
                        >
                          {st.isResidential ? 'Resident Hostel' : 'Day Scholar'}
                        </span>
                      </td>
                      <td className="py-3.5 px-4 text-right space-x-2">
                        <Link
                          href={`/portal/admin/id-cards?studentId=${st.id}`}
                          className="px-2.5 py-1 rounded-lg bg-gold-50 hover:bg-gold-100 text-gold-800 font-bold border border-gold-200 inline-flex items-center gap-1"
                        >
            ...
            <div className="bg-white rounded-3xl p-6 sm:p-8 max-w-2xl w-full border border-slate-200 shadow-2xl space-y-6 animate-fade-in my-8">
              ...
              <form onSubmit={handleAddSubmit} className="space-y-4 text-xs">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div className="space-y-1">
                    <label className="font-bold text-slate-700">Hostel Resident *</label>
                    <select
