...
export default function AdminSettingsPage() {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setMsg('Institutional settings and receipt configuration saved successfully!');
    } catch (err: any) {
      alert(err.message);
    } finally {
      setSaving(false);
    ...
    <PortalLayout role="SUPER_ADMIN">
      <div className="space-y-6 max-w-4xl mx-auto animate-fade-in">
        <div className="bg-white rounded-3xl p-6 sm:p-8 border border-slate-200 shadow-sm space-y-1">
          ...
          <h1 className="text-2xl font-extrabold text-brand-950">School & ERP Settings</h1>
          <p className="text-xs text-slate-500">
            Dynamically update school identity, contact information, session parameters, and receipt formats.
          </p>
        </div>
          ...
          <div className="space-y-4 pt-4 border-t border-slate-100">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div className="space-y-1 sm:col-span-3">
                <label className="font-bold text-slate-700">Street / Village / Post Address</label>
                <input
                  type="text"
              ...
              <div className="space-y-1">
                <label className="font-bold text-slate-700">Late Fee Per Day (INR)</label>
                <input
                  type="number"
