    ...
    <AdminLayout>
      <div className="space-y-6 max-w-7xl mx-auto animate-fade-in">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white rounded-3xl p-6 border border-slate-200 shadow-sm">
          <div>
            <h1 className="text-2xl font-extrabold text-brand-950">Notice Board & Official Circulars</h1>
            <p className="text-xs text-slate-500">
              Publish announcements, examination routines, fee deadlines, and holiday notices across portal channels.
            </p>
          </div>
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="bg-slate-50 border-b border-slate-200 text-slate-600 font-bold uppercase tracking-wider">
                  <th className="py-3 px-4">Circular Title & Excerpt</th>
              ...
              <tbody className="divide-y divide-slate-100 font-medium">
                {notices.map((n) => (
                  <tr key={n.id} className="hover:bg-slate-50/80 transition-colors">
                    ...
                    <td className="py-3.5 px-4 text-slate-500">{formatDate(n.publishDate)}</td>
          ...
          <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-950/60 backdrop-blur-sm flex items-center justify-center p-4">
            <div className="bg-white rounded-3xl p-6 sm:p-8 max-w-lg w-full border border-slate-200 shadow-2xl space-y-6 animate-fade-in my-8">
              <div className="flex items-center justify-between border-b border-slate-100 pb-3">
                <h3 className="text-lg font-bold text-brand-950">Publish Circular</h3>
              ...
              <form onSubmit={handlePublish} className="space-y-4 text-xs">
                ...
                <div className="pt-4 flex justify-end gap-2 border-t border-slate-100">
                  ...
                  <button type="submit" disabled={saving} className="px-5 py-2 rounded-xl bg-brand-900 text-white font-bold shadow">
                    {saving ? 'Publishing...' : 'Broadcast Circular'}
                  </button>
                </div>
              </form>
            </div>
