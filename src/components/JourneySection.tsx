...
export default function JourneySection() {
  ...
  return (
    <section id="journey" className="relative py-20 md:py-28 border-t border-white/5 bg-dark-950 overflow-hidden">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="relative mb-10 overflow-x-auto pb-4 no-scrollbar">
          <div className="flex items-center justify-between min-w-[700px] border-b border-white/10 pb-6 relative">
            {steps.map((step, idx) => {
                ...
                >
                  <div
                    className={`w-11 h-11 rounded-xl flex items-center justify-center transition-all duration-300 font-mono text-xs font-bold ${
                      isActive
                        ? 'bg-indigo-600 text-white shadow-glow-md ring-2 ring-indigo-400 scale-110'
                        : isPassed
                        ? 'bg-emerald-500/20 border border-emerald-500/30 text-emerald-400'
                        : 'bg-dark-850 border border-white/10 text-slate-400 group-hover:border-indigo-500/40 group-hover:text-white'
                    }`}
                  >
                  ...
                  <span
                    className={`text-[11px] font-mono tracking-wider transition-colors ${
                      isActive ? 'text-indigo-400 font-bold' : 'text-slate-400 group-hover:text-slate-200'
                    }`}
                  >
                    {step.name}
                  </span>
                  ...
                  {idx < steps.length - 1 && (
                    <div className="hidden lg:block absolute left-[calc(100%+0.5rem)] top-5 w-[calc(100%-2rem)] h-px bg-white/10 -z-0" />
                  )}
                </button>
              );
            })}
        ...
        <GlassCard className="p-6 sm:p-8 md:p-10 border-indigo-500/30" glowColor="indigo">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
            {/* Left Info */}
            <div className="lg:col-span-7 space-y-6">
              <div className="flex flex-wrap items-center gap-3">
                <span className="px-3 py-1 rounded-md bg-indigo-500/10 border border-indigo-500/30 text-indigo-400 text-xs font-mono font-bold">
              ...
              <div className="space-y-2.5 pt-2">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                  {current.deliverables.map((item, i) => (
                    <div
                      key={i}
                      className="flex items-start gap-2.5 p-3 rounded-lg bg-dark-850/80 border border-white/5 text-xs text-slate-200"
                    >
            ...
            <div className="lg:col-span-5 flex flex-col justify-between p-6 rounded-xl bg-dark-850 border border-white/10 space-y-6">
